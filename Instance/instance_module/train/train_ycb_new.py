...
from lib.testnew import SEG
from datasets.ycb.ycb_dataset_seg import YCB_Dataset
...
def load_checkpoint(model=None, optimizer=None, filename="checkpoint"):
    ...
    if model is not None and checkpoint["model_state"] is not None:
        ...
        # path = 'train_log/ycb/seg_0.03306916002349897.pth'
...
def model_fn_decorator(
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            rgb, pcld, choose, labels, idx, ctr_targ_ofst = cu_dt

            pred_rgbd_seg, pred_ctr_of = model(rgb, pcld, choose)
            #pred_rgbd_seg = model(rgb, choose)
            ...
            loss = 2 * loss_rgbd_seg + loss_ctr_of
            #loss =  loss_rgbd_seg
            acc_rgbd = (
                classes_rgbd == labels
            ...
            # print("pred_seg:", torch.unique(classes_rgbd))
            # for i in range(len(torch.unique(labels))):
            #     print(torch.sum(labels == torch.unique(labels)[i]))
            #     print(torch.sum(classes_rgbd == torch.unique(labels)[i]))
            #     if torch.unique(labels)[i] == 19 or torch.unique(labels)[i] == 20 or torch.unique(labels)[i] == 17:
            #         print(len(((labels == torch.unique(labels)[i]) * (classes_rgbd == torch.unique(labels)[i])).nonzero()) / torch.sum(classes_rgbd == torch.unique(labels)[i]))
            #
                ...
                for ibs, batch in enumerate(train_loader):
                    ...
                    if eval_flag:
                        ...
                        if test_loader is not None:
                            ...
                            torch.save(self.model.state_dict(), 'train_log/ycb/seg_{0}.pth'.format(val_loss))
...
if __name__ == "__main__":
    ...
    model = SEG(
        num_classes=22, num_points=config.n_sample_points
    ).cuda()
    ...
    # state_dict = torch.load(os.path.join('train_log/ycb','seg_0.08423834332091323.pth'))
    #model.load_state_dict(state_dict)
    #torch.save(net.state_dict(), os.path.join('train_log/ycb','seg_0.0842.pth'),
     #      _use_new_zipfile_serialization=False)
    cur_mdl_pth = os.path.join(config.log_model_dir, 'test/pvn3d_best.pth.tar')
    ...
    trainer = Trainer(
        ...
        model_fn,
        optimizer,
        checkpoint_name = os.path.join(checkpoint_fd, "test/pvn3d"),
        best_name = os.path.join(checkpoint_fd, "test/pvn3d_best"),
