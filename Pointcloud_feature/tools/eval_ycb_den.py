...
from lib.network_attention_mul_1 import PoseNet#, PoseRefineNet
...
result_wo_refine_dir = 'experiments/eval_result/ycb-mul/Densefusion_wo_refine_result'
result_refine_dir = 'experiments/eval_result/ycb-mul/Densefusion_iterative_result'
...
for now in range(2177, 2949):
    img = Image.open('{0}/{1}-color.png'.format(opt.dataset_root, testlist[now]))
    depth_ori = np.array(Image.open('{0}/{1}-depth.png'.format(opt.dataset_root, testlist[now])))
    ...
    msk_dp = depth_ori > 1e-6
    choose = msk_dp.flatten().nonzero()[0].astype(np.uint32)
    label_choose = label.flatten()[choose]
    depth = depth_ori.flatten()[choose]
    ...
    label_choose = label_choose[choose_2].astype(np.int32)
    ...
    pred_rgbd_seg, pred_ctr,_ = seg_estimator(img_masked, pcld, choose)
    #pred_rgbd_seg = seg_estimator(img_masked, choose)
    _, classes_rgbd = torch.max(pred_rgbd_seg[0], -1)
    mask, ctrs = cluster_mask(pcld, classes_rgbd, pred_ctr,label)
    label_1 = label.flatten()
    label_1[choose] = mask.cpu().numpy()
    label_1 = label_1.reshape(480, 640)

    for idx in range(len(lst)):
        itemid = lst[idx]
        try:
            mask_depth = ma.getmaskarray(ma.masked_not_equal(depth_ori, 0))
            mask_label = ma.getmaskarray(ma.masked_equal(label, itemid))
            mask_label_1 = ma.getmaskarray(ma.masked_equal(label_1, itemid))
            mask = mask_label_1 * mask_depth
            ...
            depth_masked = depth_ori[rmin:rmax, cmin:cmax].flatten()[choose][:, np.newaxis].astype(np.float32)
            ...
            pred1_r, pred1_t, pred1_c, pred2_r, pred2_t, pred2_c, pred3_r, pred3_t, pred3_c, emb = estimator(img_masked,
                                                                                                             cloud,
                                                                                                             choose,
                                                                                                             index)

            pred_r = torch.cat([pred1_r, pred2_r, pred3_r], dim=1)
            pred_t = torch.cat([pred1_t, pred2_t, pred3_t], dim=1)
            pred_c = torch.cat([pred1_c, pred2_c, pred3_c], dim=1)

            point1 = cloud.view(bs, 500, 1, 3)
            point2 = cloud.view(bs, 500, 1, 3)[:, 0:250, :, :]
            point3 = cloud.view(bs, 500, 1, 3)[:, 0:125, :, :]
            point = torch.cat([point1, point2, point3], dim=1)

            pred_r = pred_r / torch.norm(pred_r, dim=2).view(1, 875, 1)
            pred_c = pred_c.view(bs, 875)
            how_max, which_max = torch.max(pred_c, 1)
            pred_t = pred_t.view(bs * 875, 1, 3)

            my_r = pred_r[0][which_max[0]].view(-1).cpu().data.numpy()
            my_t = (point.view(bs * 875, 1, 3) + pred_t)[which_max[0]].view(-1).cpu().data.numpy()
