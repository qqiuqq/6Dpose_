...
def loss_r(pred_r, pred_c, target, model_points, w=0.015, num_point_mesh=2600):
    # knn = KNearestNeighbor(1)
    bs, num_p, _ = pred_c.size()
    ...
    pred = torch.bmm(model_points, base)
...
class Loss(_Loss):
    ...
    # def forward(self, pred_r, pred_c, target, model_points):
    #
    #     return loss_r(pred_r, pred_c, target, model_points)
