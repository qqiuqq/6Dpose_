...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        choose = mask[rmin:rmax, cmin:cmax]
        choose = choose.flatten().nonzero()[0]
        if len(choose) == 0:
        ...
        a, b, c = img_masked.shape
        img = img_masked.reshape(a, b*c)
        choose_img = np.array(choose).reshape(1,500)
        choose_img = choose_img.repeat(a, axis=0)
        x, y = img.shape
        img = torch.from_numpy(img)
        choose_img = torch.from_numpy(choose_img)
        img = torch.gather(img, 1, choose_img)
        img = img.view(x, 20, 25)
        img = img.numpy()
        cam_scale = 1.0
